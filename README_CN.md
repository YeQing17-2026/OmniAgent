...
| :---- | :---- | :---- | :---- |
| **Skill 进化** | 静态无进化 | 执行后**定期**进化（慢） | 执行中**实时**自进化（快） |
...
- [ ] **四层主动记忆体系（Proactive Memory 2.0）** — 从对话中自动提取和沉淀长期记忆，完善主动式记忆体系，解决自进化间冲突与优先级问题
- [ ] **Plan-Mode 模式** — 实现全新的 Agent 规划模式，在 Agent 执行复杂任务前生成并确认方案，再逐步执行
- [ ] **扩展渠道生态** — 新增微信、企微、大象、钉钉 等渠道接入，完善渠道抽象层，降低新渠道接入成本
